...
const ChatHistory = ({
  ...
  // Group: Today vs Previous
  const now = new Date();
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());

  const today: GeneralChat[] = [];
  const previous: GeneralChat[] = [];
  chats.forEach((chat) => {
  ...
  const renderChat = (chat: GeneralChat) => {
    const isEditing = editingId === chat.id;
    return (
      <div
        key={chat.id}
        className={cn(
          "group flex items-start gap-1 rounded-md px-2 py-1.5 text-xs transition-colors",
        ...
        {isEditing ? (
          ...
          <>
            <button
              ...
              className="flex-1 text-left min-w-0 break-words whitespace-normal leading-snug"
  ...
  return (
    <div className="mt-1">
      ...
      {expanded && (
        <div className="ml-4 mt-1 border-l border-border pl-2 max-h-[50vh] overflow-y-auto scrollbar-thin">
          <button
          ...
          {chats.length === 0 ? (
            <p className="px-3 py-2 text-[11px] text-muted-foreground">No conversations yet</p>
          ) : (
            <>
              ...
              {today.length > 0 && (
                <div className="mb-2">
                  <button
                    ...
                    className="flex w-full items-center gap-1 px-3 py-1 text-[10px] font-mono uppercase tracking-wider text-muted-foreground/70 hover:text-foreground transition-colors"
                  >
                    <ChevronDown className={cn("h-3 w-3 transition-transform", !todayOpen && "-rotate-90")} />
                    <span>Today</span>
                    <span className="ml-auto text-muted-foreground/50">{today.length}</span>
                  </button>
                  {todayOpen && <div className="space-y-0.5">{today.map(renderChat)}</div>}
                </div>
              )}

              {/* Previous (collapsed by default) */}
              {previous.length > 0 && (
                <div className="mb-2">
                  <button
                    ...
                    className="flex w-full items-center gap-1 px-3 py-1 text-[10px] font-mono uppercase tracking-wider text-muted-foreground/70 hover:text-foreground transition-colors"
                  >
                    <ChevronDown className={cn("h-3 w-3 transition-transform", !previousOpen && "-rotate-90")} />
                    <span>Previous</span>
                    <span className="ml-auto text-muted-foreground/50">{previous.length}</span>
                  </button>
                  {previousOpen && <div className="space-y-0.5">{previous.map(renderChat)}</div>}
                </div>
              )}
            </>
          )}
