...
const HomeContext = () => {
  ...
  const upcoming = events
    .filter(e => new Date(e.start_at || 0) >= now)
          ...
          upcoming.map(e => (
            <Row
              ...
              right={<span className="text-[10px] text-muted-foreground">
                {new Date(e.start_at || 0).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
              </span>}
...
const PlannerContext = () => {
  ...
  const todays = events.filter(e => new Date(e.start_at || 0).toDateString() === todayKey);
  const upcoming = events
    .filter(e => new Date(e.start_at || 0) > today && new Date(e.start_at || 0).toDateString() !== todayKey)
    .sort((a, b) => +new Date(a.start_at || 0) - +new Date(b.start_at || 0))
    .slice(0, 5);
        ...
        {todays.length === 0 ? (
          ...
          todays.map(e => (
            <Row
              ...
              label={e.title}
              right={<span className="text-[10px] text-muted-foreground">
                {new Date(e.start_at || 0).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}
              </span>}
            />
          ))
        )}
