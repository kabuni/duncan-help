...
function ProgressCell({ item, isAdmin, onUpdate }: { item: Plan90Deliverable; isAdmin: boolean; onUpdate: Props["onUpdate"] }) {
  const [open, setOpen] = useState(false);
  const [val, setVal] = useState<string>(item.progress_percent == null ? "" : String(item.progress_percent));
  useEffect(() => { setVal(item.progress_percent == null ? "" : String(item.progress_percent)); }, [item.progress_percent]);
  ...
  async function save() {
    ...
    if (trimmed === "") {
      await onUpdate(item.id, { progress_percent: null });
    } else {
      const n = Math.max(0, Math.min(100, Math.round(Number(trimmed))));
      if (Number.isNaN(n)) { toast.error("Enter 0–100"); return; }
      if (item.status === "Completed" && n < 100) {
        // Trigger will require re-entry; forbid silently making Completed inconsistent
        toast.error("Change status off Completed before lowering progress");
        return;
      }
      await onUpdate(item.id, { progress_percent: n });
    }
    setOpen(false);
  }

  const trigger = (
    <button type="button" disabled={!isAdmin} className={cn("w-full flex items-center gap-2 text-left group", isAdmin && "cursor-pointer")}>
      <div className="h-1.5 flex-1 rounded-full bg-secondary overflow-hidden">
        <div className={cn("h-full transition-all", barColor)} style={{ width: `${pct ?? 0}%` }} />
      </div>
      <span className={cn("text-[11px] tabular-nums whitespace-nowrap", unknown ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground")}>
        {unknown ? "—" : `${pct}%`}
      </span>
    </button>
  );
  ...
  if (!isAdmin) {
    return <div title={unknown ? "Progress not yet updated" : `${pct}%`}>{trigger}</div>;
  }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className="w-56 p-3 space-y-2">
        <div className="text-[11px] text-muted-foreground">Progress % (0–100)</div>
        <Input
        ...
        <div className="flex justify-between gap-2">
          <Button size="sm" variant="ghost" onClick={() => { setVal(""); }}>Clear</Button>
          <div className="flex gap-1">
            <Button size="sm" variant="ghost" onClick={() => setOpen(false)}>Cancel</Button>
            <Button size="sm" onClick={save}>Save</Button>
          </div>
        </div>
        {unknown && <div className="text-[10px] text-amber-600 dark:text-amber-400">Not yet updated — currently counted as 0% in the average.</div>}
      </PopoverContent>
    </Popover>
  );
}
