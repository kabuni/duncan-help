...
export default function MyFeatureRequestsList() {
  ...
  if (requests.length === 0) {
    return <p className="text-xs text-muted-foreground py-4">No requests yet. Submit one from the form above.</p>;
  }

  return (
    <div className="grid grid-cols-1 md:grid-cols-[minmax(220px,280px)_1fr] gap-3">
      <aside className="rounded-xl border border-border bg-card/60 p-2 max-h-[60vh] overflow-y-auto">
            ...
            return (
              <li key={r.id}>
                <button
                  ...
                  className={`w-full text-left rounded-lg p-2.5 transition-colors ${activeId === r.id ? "bg-secondary/70" : "hover:bg-secondary/40"}`}
                >
      ...
      <section className="rounded-xl border border-border bg-card/60 p-4 min-h-[300px]">
        {!active ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              {active.admin_notes && active.triage_status === "dismissed" && (
                <p className="mt-2 text-[11px] text-muted-foreground italic border-l-2 border-border pl-2">
                  Duncan's note: {active.admin_notes}
                </p>
              )}
              {active.workstream_card_id && (
                <a href="/workstreams" className="mt-1 inline-flex items-center gap-1 text-[11px] text-primary hover:underline">
            ...
            <div className="border-t border-border pt-3">
              <h5 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-2">Conversation</h5>
              {threadLoading ? (
                ...
                <ol className="space-y-2">
                  {thread.map((m) => (
                    <li key={m.id} className={`rounded-lg p-2.5 ${m.role === "agent" ? "bg-primary/5 border border-primary/15" : "bg-secondary/50"}`}>
            ...
            {active.triage_status !== "filed" && active.triage_status !== "dismissed" && (
              <div className="border-t border-border pt-3 space-y-2">
                <label className="text-[11px] font-medium text-muted-foreground">Reply to Duncan</label>
                <textarea
                  value={reply}
                  onChange={(e) => setReply(e.target.value)}
                  rows={3}
                  placeholder="Answer Duncan's questions or add more context…"
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-ring resize-y"
                />
