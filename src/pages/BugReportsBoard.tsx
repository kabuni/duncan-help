    ...
    <main className="flex-1 overflow-y-auto p-4 sm:p-8">
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              <Bug className="h-5 w-5 text-muted-foreground" /> Bug Reports
            </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Stat label="Total reported" value={isLoading ? "…" : total} />
          <Stat label="Fixed" value={isLoading ? "…" : fixed} className="text-emerald-600 dark:text-emerald-400" />
          <Stat label="Open" value={isLoading ? "…" : open} className="text-amber-600 dark:text-amber-400" />
          <Stat label="Resolution rate" value={isLoading ? "…" : `${rate}%`} />
        </div>
        ...
        {error ? (
          ...
          <p className="text-sm text-muted-foreground py-10 text-center">No bug reports found.</p>
        ) : (
          <div className="rounded-xl border border-border bg-card divide-y divide-border/60">
            {visible.map((r) => (
              <div key={r.id} className="p-4 space-y-1.5">
                ...
                <p className="text-xs text-foreground/80 whitespace-pre-wrap line-clamp-3">{r.description}</p>
                {isAdmin && (
                  <div className="pt-1">
                    <button
                      onClick={() => toggleResolved(r)}
                      className="rounded-md border border-border bg-background px-2.5 py-1 text-[11px] text-foreground hover:bg-secondary/60 transition-colors"
                    >
