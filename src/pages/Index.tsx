                ...
                <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                  a: ({ href, children }) => {
                    const isDownloadLink = href && (
                      (href.includes("azure-blob-api") && (href.includes("blob_path") || href.includes("path="))) ||
                      href.includes("/nda/download/")
                    );
                    if (isDownloadLink && href) {
                      // Resolve relative API paths to the full backend URL
                      const fullHref = href.startsWith("/") ? `${import.meta.env.VITE_API_BASE_URL || ""}${href}` : href;
                      const isDownloading = downloadingUrl === fullHref;
                      return (
                        <button onClick={() => handleAuthenticatedDownload(fullHref)} disabled={isDownloading} className="inline-flex items-center gap-1.5 text-primary hover:text-primary/80 underline underline-offset-2 font-medium disabled:opacity-50">
...
const Index = () => {
  ...
  const runBriefing = useCallback(async () => {
    ...
    try {
      ...
      const apiBase = import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_SUPABASE_URL}/functions/v1`;
      const resp = await fetch(
        `${apiBase}/daily-briefing`,
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    ...
    const handleAuthChange = async (e: Event) => {
      ...
      if (!detail?.loggedIn || briefingTriggered.current) return;
      await runBriefing();
    };
    window.addEventListener("duncan-auth-change", handleAuthChange);

    // Also run immediately if already signed in
    (async () => {
      if (briefingTriggered.current) return;
      const token = getAuthToken();
      if (!token || briefingTriggered.current) return;
      await runBriefing();
    })();

    return () => window.removeEventListener("duncan-auth-change", handleAuthChange);
  }, [runBriefing]);
  ...
  const handleAuthenticatedDownload = useCallback(async (url: string) => {
    try {
      ...
      const response = await fetch(url, { headers: { Authorization: `Bearer ${token}`, "ngrok-skip-browser-warning": "1" } });
