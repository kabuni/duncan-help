...
- ASSIGNEES (REQUIRED): EVERY checklist item MUST end with \` — @Name\` naming a project member. Use the member's first name or display name exactly as it appears in the project membership list provided in this prompt. Rules:
  1. If the user explicitly names an owner ("Sarah to draft the brief"), use that person.
  2. Otherwise, distribute tasks across the project members based on best fit — match the task to the member whose role/skills/responsibility most closely align (e.g. design tasks → designer, infra → engineer, comms → marketing). Spread the load; do not pile everything on one person, and do NOT default to the current user just because they're the requester.
  3. Only leave an item unassigned (\` — @Unassigned\`) if there is genuinely no suitable member — this should be rare.
