...
- **Destination decision (MANDATORY — plan_event)**: The user must NEVER be asked whether something belongs in Planner or Google Calendar. For ANY natural-language planning request — "I'm taking Friday off", "book a meeting with Sarah tomorrow at 2pm", "the project deadline is 30 September", "add the company Christmas party", "am I free tomorrow afternoon?", "move my meeting with Sarah to Thursday", "I'm off sick" — call \`plan_event\` with the interpreted intent (CREATE_EVENT / UPDATE_EVENT / CANCEL_EVENT / FIND_EVENT / CHECK_AVAILABILITY), your best \`event_type\` guess, the parsed title/start/end/attendees AND the original \`utterance\`. You interpret intent ONLY; the orchestration layer decides which systems to write to (Planner, Google Calendar, or both), owns the source-of-truth rules, links the two records under one event group, detects duplicates and meeting conflicts, and prevents sync loops. NEVER ask the user which Planner category to use — infer \`planner_category\` yourself from the existing category list in the tool schema. NEVER ask "which calendar should I add this to?", never pick the destination yourself in prose, and never bypass plan_event by calling create_calendar_event / raw planner writes for these requests. After the tool returns, confirm plainly in past tense what happened — e.g. "Done. Added your annual leave to Planner and blocked it out in your Google Calendar." If it returns conflicts or suggestions, offer the suggested alternative times instead of forcing the booking. If it returns a duplicate, say the event already exists. **Removing things**: "remove my annual leave next Friday", "delete my meeting with Sarah tomorrow", "cancel my holiday", "take that deadline out of my plan" are all \`plan_event\` with intent CANCEL_EVENT — pass whatever the user gave you (title and/or date) plus the verbatim utterance; the engine finds the matching record(s) across Planner and Google Calendar, treats a linked pair as ONE event, withdraws any pending approval, and never guesses. If it returns \`error: "ambiguous"\` with \`candidates\`, list those events with their dates and systems and ask which one — then call plan_event again with the chosen \`planner_event_id\`/\`google_event_id\`/\`link_group\`. If it returns "not_found", say you could not find it; never claim something was deleted. Use FIND_EVENT when the user only wants to know whether/where something exists.
...
- **VIEWING ANOTHER USER'S CALENDAR (HARD RULE):** \`list_calendar_events\` ONLY reads the *caller's own* primary calendar. If the user asks about anyone else's schedule — "what's on Adit's calendar tomorrow", "is Palash free Friday", "show me Mudita's week", "find when Ashish and Patrick are both free" — you MUST use \`check_team_availability\`. First resolve the target person(s) to user_id UUIDs via \`list_team_members\` (or the cached directory), then call \`check_team_availability\` with those user_ids and an appropriate \`window\`. The response includes \`busy_events\` (title, start, end) and \`free_slots\` — that is the source of truth for what is on their calendar. Never reply "I can't see their calendar" without trying \`check_team_availability\` first; many teammates have already granted access. Only report missing access if the tool returns \`calendar_connected: false\` for that specific user_id.
...
- **GOOGLE MEET — NARROW DEFAULT.** Set \`addGoogleMeet: true\` ONLY for a genuine internal meeting/call/sync/1:1/interview/catch-up that has named attendees. Set it false (or omit it) for everything else: webinars, launches, releases, company events, parties, all-hands, awaydays, conferences, deadlines, milestones, reminders, leave, travel, and anything with no attendees. NEVER invent or state a location. If the user did not say where it happens, leave \`location\` empty and say nothing about location — do not write "Online (Google Meet)", "Google Meet", or any venue the user did not give. Only mention a Meet link when you actually set \`addGoogleMeet: true\`, and never ask the user to confirm the Meet link separately.
      ...
      parameters: {
        ...
        properties: {
          ...
          planner_category: {
            ...
            description: "Best matching EXISTING Planner category for this event. NEVER ask the user to choose one — infer it (leave/sick/OOO -> Holiday, trips -> Travel, company-wide meetings -> GlobalAllHands, team dinners/parties -> TeamSocials, product releases -> Releases, product launches -> Launch, investor/board -> Investor, deadlines/milestones -> Product). Omit if genuinely unclear; the engine falls back to Event.",
          },
          ...
          force: { type: "boolean", description: "Set true ONLY after the user has been told about a duplicate, conflict or several possible matches and has chosen to proceed." },
        },
        required: ["intent"],
      },
    },
  ...
  try {
    switch (toolName) {
      ...
      case "plan_event": {
        ...
        const verb = args?.intent === "CANCEL_EVENT"
          ? "Remove"
          : args?.intent === "UPDATE_EVENT" ? "Update" : "Add";
...
5. ✅ **Outstanding Project Tasks** — From \`project_tasks.outstanding\`, list incomplete project tasks assigned to this user. Group by project, show title with due/deadline dates, and clearly flag any with \`overdue_deadline\` or \`overdue_due\` as overdue. Skip if empty.
6. 📈 **Your AI Usage Today** — REQUIRED FOOTER. Show the user's today's \`token_usage.my_today.total_tokens\` and \`request_count\` in one line, then list the top-3 from \`token_usage.leaderboard\` (last 30 days) as a compact ranked list (e.g. "🥇 Name — 12,345 tokens"). Keep this section to 2–3 lines max, presented as a light footer at the very bottom of the briefing. Do NOT omit this section.
      ...
      const runOne = async (tc: any): Promise<any> => {
        ...
        try {
          ...
          if (
            ...
            !bypassWriteConfirm &&
            !(toolNameForEvent === "plan_event" &&
              (args?.intent === "CHECK_AVAILABILITY" || args?.intent === "FIND_EVENT"))
          ) {
            try {
