...
Format as a natural, readable summary with clear sections. If a section has no data, briefly note "No updates since last check-in" for that area. Keep it actionable and concise. Address the user by name. Highlight anything urgent (overdue items, items due today). For returning check-ins, emphasize what's new or changed.`;
    ...
    const INTENT_RULES: Array<{ groups: any[][]; re: RegExp }> = [
      ...
      { groups: [CALENDAR_TOOLS], re: /\b(calendar|diary|schedule|scheduling|availability|free\/busy|free busy|book\b|booking|meeting room|reschedule|invite|invites|event|events|appointment|set\s+(?:up\s+)?(?:a|an|the)?\s*(?:meeting|call|catch[- ]?up|sync|chat|1:1|one[- ]?on[- ]?one)|arrange\s+(?:a|an|the)?\s*(?:meeting|call|catch[- ]?up|sync)|put\s+(?:it|this|that|.+?)\s+on\s+(?:my|the|our)\s+calendar|block\s+(?:time|out)|find\s+time|google\s*meet|meet\s+link|zoom\s+link|teams\s+link)\b/i },
