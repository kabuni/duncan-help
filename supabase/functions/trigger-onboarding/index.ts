      ...
      if (accessToken) {
        ...
        const events = [
          ...
          },
          {
            ...
            start: { dateTime: ymd(addDays(start, 30)) + "T14:00:00Z" === "" ? "" : new Date(addDays(start, 30).toISOString().slice(0, 10) + "T14:00:00Z").toISOString(), timeZone: "UTC" },
            end:   { dateTime: new Date(addDays(start, 30).toISOString().slice(0, 10) + "T15:00:00Z").toISOString(), timeZone: "UTC" },
            description: `30-day review with ${fullName}.\nOnboarding card: ${card.id}`,
            attendees,
          },
          {
            summary: `90-Day Review — ${fullName}`,
            start: { dateTime: new Date(addDays(start, 90).toISOString().slice(0, 10) + "T14:00:00Z").toISOString(), timeZone: "UTC" },
            end:   { dateTime: new Date(addDays(start, 90).toISOString().slice(0, 10) + "T15:00:00Z").toISOString(), timeZone: "UTC" },
